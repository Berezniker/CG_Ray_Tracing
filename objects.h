...
#include <cmath>     // M_PI, std::fmin(), sqrtf(), atan2f()
...
struct Material {
    ...
    explicit Material(MATERIAL_TYPE materialType, const float3 &color,
                      float specularPower, const std::vector<float> &albedo, float eta=1.0f) :
            materialType(materialType), color(color), specularPower(specularPower), eta(eta)
    { set_albedo(albedo); }

    explicit Material(MATERIAL_TYPE materialType, const char *texturePath,
                      float specularPower, const std::vector<float> &albedo, float eta=1.0f) :
            materialType(materialType), color(),  specularPower(specularPower), eta(eta) {
        set_albedo(albedo);
    ...
    void set_albedo(const std::vector<float> &initAlbedo) {
        ...
        switch (materialType) {
            case GLASS:
                albedo[3] = initAlbedo[3];
            case MIRROR:
                albedo[2] = initAlbedo[2];
            case FONG:
                albedo[1] = initAlbedo[1];
            case LAMBERT:
                albedo[0] = initAlbedo[0];
            case NORMAL:
                break;
        }
    }
...
struct Square : Plane {
    ...
    Square(Material m, const float3 &normal, const float3 &p, float height, float width) :
           Plane(std::move(m), normal.normalize(), p), height(height), width(width) {}
...
struct Cylinder : Object {
    ...
    bool is_ray_intersect(const Ray &ray, float &t, float3 &P, float3 &N, float3 &color) const override {
        ...
        t0 = fmin((-b - sqrtf(D)) / a, (-b + sqrtf(D)) / a);
...
struct Hyperboloid : Object {
    ...
    Hyperboloid(Material m, float a, float b, float c, float radius, float3 pos) :
        Object(std::move(m)), radius(radius), position(pos),
        abc2(1.0f / (a * a), -1.0f / (b * b), 1.0f / (c * c)) {}

    bool is_ray_intersect(const Ray &ray, float &t, float3 &P, float3 &N, float3 &color) const override {
        float3 center = ray.origin - position;
        float A = float3(ray.direction.x * ray.direction.x, ray.direction.y * ray.direction.y, ray.direction.z * ray.direction.z) * abc2;
        float B = float3(center.x * ray.direction.x, center.y * ray.direction.y, center.z * ray.direction.z) * abc2;
        float C = float3(center.x * center.x, center.y * center.y, center.z * center.z) * abc2 - radius;
        float D = B * B - A * C;
        if (D < 0) return MISS;
        t = std::fmin((-B - sqrtf(D)) / A, (-B + sqrtf(D)) / A);
        if (t < 0) return MISS;
    ...
    float3 get_normal(const float3 &hit) const {
        return 2 * float3(hit.x * abc2.x, hit.y * abc2.y, hit.z * abc2.z);
    }
};
